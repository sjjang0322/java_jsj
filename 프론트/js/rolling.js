...
function rollingToLeft(parentSelector, childSelector, moveTime, animationTime){
	var width = $(parentSelector).width();
	return setInterval(function(){					
		if(!$(parentSelector+'>'+childSelector).first().is(':animated')){
			$(parentSelector+'>'+childSelector).first().animate({marginLeft:-	width},animationTime,function(){
				$(this).removeAttr('style').detach().appendTo(parentSelector);
			});
		}					
	}, moveTime);
