-- 20211600001학번 학생이 1번 강의인 컴퓨터 개론 3분반 강의를 신청했다. 이 때 알맞은 쿼리를 작성하세요.
insert into course(co_st_num, co_su_num) values(2021160001,1);

-- 2021160015학번 학생이 컴퓨터 개론 3분반 강의를 신청했다. 이때 알맞은 뭐리를 작성하세요.
...
select count(*) as '홍길동 학생의 수강 과목의 수' from course
	join student on st_num = co_st_num
	where st_name = '홍길동';

-- 컴퓨터 개론 3분반 현재 수강 신청 학생수를 확인
select count(*) as '컴퓨터 개론 3분반 현재 수강신청 인원' from course
