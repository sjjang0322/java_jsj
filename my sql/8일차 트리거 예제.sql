...
delimiter ;
-- 트리거 확인을 위해 buy테이블에 행을 추가
insert into buy value(3, '바나나', 50, now(), '송이', '과일상회');
-- 트리거에 의해 fruit 테이블이 변경 됐는지 확인
select * from fruit_jsj.buy;
-- 만들어진 트리거를 확인
show triggers;
...
insert into sell values(3,'바나나',3,'송이',12000,now(), '카드');
-- 트리거 동작 확인
select * from fruit;