...
- 장바구니 기능 X, 한 제품의 옵션을 하나만 선택해서 여러개 구매할 수 있다
	- A제품 빨강 L
    - A제품 파랑 L
    - 위 두 제품을 구매하려면 A제품 빨강 L을 구매 후 A제품 파랑 L제품을 구매해야한다
- 의류 구매 가능
- 배송 정보는 회원 주소로 사용(고정)
- 결제 정보 관리 X
- 구매는 회원만 가능
...
- 제품
	- 분류방법 (카테고리)
		- 대분류로 상의/하의 등으로 구분호 중분류로상의에 가디건/니트, 민소매 등으로 분류
	- 옵션
		- 색상, 사이즈는 선택가능
        - 색상은 한글로 표기
	- 동일 제품은 x
...
CREATE TABLE `goods` (
	...
	`gd_main_category`	varchar(10)	NOT NULL,
	`gd_sub_category`	varchar(10)	NOT NULL,
	`gd_price`	int	NULL,
	`gd_stock`	int	NULL,
