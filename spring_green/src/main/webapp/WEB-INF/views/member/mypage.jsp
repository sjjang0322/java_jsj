		...
		<div class="form-group">
			<div class="form-check-inline">
				<label class="form-check-label">
					<input type="radio" class="form-check-input man" name="me_gender" value="남성" <c:if test="${user.me_gender=='남성'}">checked</c:if>>남성
				</label>
			</div>
			<div class="form-check-inline">
				<label class="form-check-label">
					<input type="radio" class="form-check-input woman" name="me_gender" value="여성" <c:if test="${user.me_gender=='여성'}">checked</c:if>>여성
				</label>
			</div>
...
<script>
	$('#birth').datepicker();
	$('#birth').datepicker('option','dateFormat', 'yy-mm-dd').val('${user.me_birth_str}');
	...
	$('form').submit(function(){
		...
		if(me_pw == '' || me_address == ' '){
			return confirm('비밀번호와 주소가 입력되지 않아 이전 정보로 저장됩니다. 진행하시겠습니까?')
		}
	});
...
//	$(function(){
//		if(${user.me_gender=='남성'})
//			$('.man').click();
//		if(${user.me_gender=='여성'})
//			$('.woman').click();
//	});
</script>
</body>
